...
title=Label(win,text="Banking Automation",font=('arial',50,'bold','underline'),bg='blue')
title.pack()

dt=time.strftime("%d %b %Y,%A")
date=Label(win,text=f"{dt}",font=('arial',18,'bold'),bg='blue',fg='yellow')
...
def forgotpass_screen():
    ...
    btn_sub.place(relx=.4,rely=.4)
    
    btn_clear=Button(frm,width=5,text="clear",font=('arial',20,'bold'),bd=5,command=clear)
    btn_clear.place(relx=.55,rely=.4)
...
def newuser_screen():
    ...
    def newuser_db():
        ...
        conobj=sqlite3.connect(database="bank.sqlite")
        curobj=conobj.cursor()
        curobj.execute("insert into acn(acn_name,acn_pass,acn_email,acn_mob,acn_gender,acn_type,acn_opendate,acn_bal) values(?,?,?,?,?,?,?,?)",(name,pwd,email,mob,gender,type,opendate,bal))
        conobj.commit()
        conobj.close()
    ...
    e_mob=Entry(frm,font=('arial',20,'bold'),bd=5)
    e_mob.place(relx=.42,rely=.4)

    lbl_gender=Label(frm,text="Gender",font=('arial',20,'bold'),bg='pink')
    lbl_gender.place(relx=.3,rely=.5)

    cb_gender=Combobox(frm,values=['-----select-------','Male','Female','other'],font=('arial',20,'bold'))
    cb_gender.place(relx=.42,rely=.5)

    lbl_type=Label(frm,text="Account Type",font=('arial',15,'bold'),bg='pink')
    lbl_type.place(relx=.3,rely=.6)

    cb_type=Combobox(frm,values=['-----select-------','Savings','Current','other'],font=('arial',20,'bold'))
    cb_type.place(relx=.42,rely=.6)

    btn_sub=Button(frm,text="Submit",font=('arial',20,'bold'),bd=5,command=newuser_db)
    btn_sub.place(relx=.42,rely=.7)

    btn_clear=Button(frm,text="Clear",font=('arial',20,'bold'),bd=5,command=clear)
    btn_clear.place(relx=.56,rely=.7)
...
def welcome_screen():
    ...
    def details():
        ifrm=Frame(frm,highlightbackground='black',highlightthickness=1.5)
        ifrm.configure(bg='white')
        ifrm.place(relx=.2,rely=.1,relwidth=.65,relheight=.6)

        lbl_detscreen=Label(ifrm,text="This is Details Screen",font=('arial',20,'bold'),bg='white',fg='blue')
        lbl_detscreen.pack()

        conobj=sqlite3.connect(database="bank.sqlite")
        curobj=conobj.cursor()
        curobj.execute("select acn_opendate,acn_bal,acn_gender,acn_type,acn_mob,acn_email from acn where acn_no=?",(gacn,))
        tup=curobj.fetchone()
        conobj.close()

        lbl_opendate=Label(ifrm,text=f"Open Date:{tup[0]}",font=('arial',15,'bold'),bg='white',fg='black')
        lbl_opendate.place(relx=.2,rely=.12)

        lbl_bal=Label(ifrm,text=f"Balance:{tup[1]}",font=('arial',15,'bold'),bg='white',fg='black')
        lbl_bal.place(relx=.2,rely=.2)

        lbl_gender=Label(ifrm,text=f"Gender:{tup[2]}",font=('arial',15,'bold'),bg='white',fg='black')
        lbl_gender.place(relx=.2,rely=.28)

        lbl_type=Label(ifrm,text=f"Account Type:{tup[3]}",font=('arial',15,'bold'),bg='white',fg='black')
        lbl_type.place(relx=.2,rely=.36)

        lbl_mob=Label(ifrm,text=f"Mob No:{tup[4]}",font=('arial',15,'bold'),bg='white',fg='black')
        lbl_mob.place(relx=.2,rely=.44)

        lbl_email=Label(ifrm,text=f"Email:{tup[5]}",font=('arial',15,'bold'),bg='white',fg='black')
        lbl_email.place(relx=.2,rely=.52)
    ...
    def update():
        ...
        lbl_upscreen=Label(ifrm,text="This is Update Screen",font=('arial',20,'bold'),bg='white',fg='blue')
        lbl_upscreen.pack()
        ...
        lbl_mob.place(relx=.5,rely=.45)

        e_mob=Entry(ifrm,font=('arial',20,'bold'),bd=5)
        e_mob.place(relx=.5,rely=.55)
        ...
        btn_update.place(relx=.8,rely=0.7)
    ...
    def deposit():
        ...
        lbl_depscreen=Label(ifrm,text="This is Deposit Screen",font=('arial',15,'bold'),bg='white',fg='blue')
        lbl_depscreen.pack()
    ...
    def withdraw():
        ...
        lbl_wdscreen=Label(ifrm,text="This is Withdraw Screen",font=('arial',20,'bold'),bg='white',fg='blue')
        lbl_wdscreen.pack()
    ...
    def transfer():
        ...
        lbl_tscreen=Label(ifrm,text="This is Transfer Screen",font=('arial',20,'bold'),bg='white',fg='blue')
        lbl_tscreen.pack()
